...
_MOTIONS: tuple[MotionKind, ...] = ("zoom_in", "pan_right", "zoom_out", "pan_left")
...
_HISTORY_STEMS = ("истор", "восстан", "импер", "династ", "войн", "арм", "солдат", "битв", "револю", "xix", "xviii")
_VIDEO_STEMS = ("идет", "идут", "беж", "летит", "едет", "горит", "взрыв", "арм", "солдат", "войск", "битв", "восстан", "толп", "марш", "атак", "движ", "сраж")
_MEME_STEMS = ("вдруг", "и тут", "прикол", "шок", "жесть", "серьезно", "неожидан", "пиздец", "лол", "смеш", "офиг", "охрен", "что за")


def build_shot_plan(transcript: Transcript, audio: str | Path, *, target_scene_seconds: float = 2.1, min_scene_seconds: float = 1.25, max_scene_seconds: float = 3.2) -> ShotPlan:
    ...
    for index, (words, caption) in enumerate(zip(chunks, captions)):
        ...
        mode = _choose_visual_mode(caption, index=index, duration=words[-1].end - words[0].start)
...
def _choose_visual_mode(text: str, *, index: int, duration: float) -> VisualMode:
    ...
    if index > 0 and index % 3 == 0 and duration >= 1.6: return "video"
    return "image"
...
def _chunk_words(words: list[Word], *, target: float, minimum: float, maximum: float) -> list[list[Word]]:
    ...
    for word in words:
        ...
        if duration >= maximum or (duration >= target and _SENTENCE_END.search(word.text)) or duration >= target + 0.45:
            if duration >= minimum: chunks.append(current); current = []
    if current:
        if chunks and current[-1].end - current[0].start < minimum * 0.72: chunks[-1].extend(current)
...
def _scene_visual_hints(text: str) -> list[str]:
    lowered=text.lower(); return [phrase for stems,phrase in _SCENE_RULES if any(stem in lowered for stem in stems)]


def _global_visual_context(text: str) -> str:
    lowered=text.lower(); parts=[]
