...
USER_AGENT = "video-ai/1.1 (+https://github.com/Masek463636/video-ai)"
...
def materialize_assets(plan: ShotPlan, out_dir: str | Path, *, limit: int = 20, overwrite: bool = False, semantic: bool = False, semantic_top_k: int = 6, replace_scenes: set[int] | None = None, rank_offset: int = 0, meme_dir: str | Path | None = None) -> list[dict]:
    ...
    for index,scene in enumerate(plan.scenes):
        ...
        if scene.asset and not overwrite and not force_replace and Path(scene.asset).exists():
            _apply_focus(scene,Path(scene.asset)); manifest.append({"scene":index,"status":"existing","path":scene.asset}); continue
        ...
        chosen=None; target=None; search_used=""; judge_info=None; rejected:list[dict]=[]; gemini_checks=0
        max_checks=6 if scene.semantic_lock else 3
        for candidate,search in ranked[max(0,rank_offset):]:
            ...
            if gemini is not None and gemini_checks<max_checks:
                gemini_checks+=1; judgement=gemini.judge_visual(scene,candidate_target,candidate_title=candidate.title,source=candidate.source)
                if judgement is not None:
                    judge_info={"score":judgement.score,"accept":judgement.accept,"reason":judgement.reason,"mismatch":judgement.mismatch}
                    if not judgement.accept:
                        rejected.append({"title":candidate.title,"source":candidate.source,"score":judgement.score,"reason":judgement.reason,"mismatch":judgement.mismatch}); candidate_target.unlink(missing_ok=True); continue
                    candidate.score+=judgement.score/12.0
            chosen,target,search_used=candidate,candidate_target,search; break

        # Locked facts are never allowed to bypass Gemini after rejection.
        if chosen is None and ranked and not scene.semantic_lock:
            for candidate,search in ranked[max(0,rank_offset)+gemini_checks:]:
                if not _source_allowed(scene,candidate): continue
                suffix=_suffix(candidate); candidate_target=out_dir/f"scene_{index:03d}{suffix}"
                try:
                    if candidate.local_path: shutil.copy2(candidate.local_path,candidate_target)
                    else: _download(candidate.download_url,candidate_target)
                    if candidate_target.exists() and candidate_target.stat().st_size>=1024:
                        chosen,target,search_used=candidate,candidate_target,search; judge_info={"fallback_after_rejections":True}; break
                except Exception: candidate_target.unlink(missing_ok=True)
        ...
        if chosen is None or target is None:
            ...
            manifest.append({"scene":index,"status":"not_found_locked" if scene.semantic_lock else "not_found","query":scene.query,"visual_mode":scene.visual_mode,"source_mode":scene.source_mode,"semantic_lock":scene.semantic_lock,"required_entities":scene.required_entities,"required_context":scene.required_context,"semantic_fallback":scene.semantic_fallback,"motion_preset":scene.motion_preset,"visual_description":scene.visual_description,"queries_tried":variants,"gemini_rejected":rejected}); continue
        ...
        manifest.append({"scene":index,"status":"downloaded","query":scene.query,"visual_mode":scene.visual_mode,"source_mode":scene.source_mode,"semantic_lock":scene.semantic_lock,"required_entities":scene.required_entities,"required_context":scene.required_context,"semantic_fallback":scene.semantic_fallback,"motion_preset":scene.motion_preset,"meme_filename":scene.meme_filename,"visual_description":scene.visual_description,"queries_tried":variants,"search_used":search_used,"path":str(target),"source":chosen.source,"kind":chosen.kind,"gemini_judge":judge_info,"gemini_rejected":rejected,"top_candidates":[{"title":c.title,"score":round(c.score,3),"search":q,"source":c.source,"kind":c.kind,"semantic_score":round(c.semantic_score,4) if c.semantic_score is not None else None} for c,q in ranked[:8]],**asdict(chosen)})
...
def ensure_visual_coverage(plan: ShotPlan, manifest: list[dict] | None = None) -> set[int]:
    ...
    for index,scene in enumerate(plan.scenes):
        ...
        if scene.semantic_lock:
            ...
            compatible=[i for i in good if plan.scenes[i].source_mode==scene.source_mode or scene.source_mode in {"auto","generic_image"}] or good
        ...
        if index in by_scene: by_scene[index].update({"status":"fallback_nearest","fallback_from_scene":source_index,"path":scene.asset})
    return filled
...
def _query_variants(scene: Scene) -> Iterable[str]:
    ...
    if scene.semantic_lock and locked:
        base += [f"{locked} {scene.visual_description or ''}",f"{locked} archival photograph",f"{locked} historical illustration",f"{locked} map",scene.semantic_fallback or ""]
    ...
    elif scene.visual_mode=="video": base += [f"{scene.visual_description or scene.query} documentary footage",f"{scene.visual_description or scene.query} b roll"]
    elif scene.visual_mode=="meme": base += [f"{scene.visual_description or scene.query} reaction"]
    else: base += [f"{scene.visual_description or scene.query} photo illustration"]
    for value in base:
...
def _semantic_rerank(scene: Scene, ranked: list[tuple[AssetCandidate,str]], *, top_k:int)->None:
    ...
    prompt=" ".join([*(scene.required_entities if scene.semantic_lock else []),*(scene.required_context if scene.semantic_lock else []),scene.visual_description or scene.query or "documentary scene"]).strip()
