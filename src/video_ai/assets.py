    ...
    *,
    ...
    for index, scene in enumerate(plan.scenes):
            ...
            for meme in meme_candidates:
                ...
                pool[key] = (
                    AssetCandidate(
                        ...
                        page_url="",
                        download_url=key,
                        mime="video/mp4" if meme.kind == "video" else "image/jpeg",
                        width=0,
                        height=0,
                        size=meme.path.stat().st_size,
                        kind=meme.kind,
        ...
        for candidate, search in ranked[max(0, rank_offset):]:
            ...
            if gemini is not None and gemini_checks < 3:
                gemini_checks += 1
            ...
            for candidate, search in ranked[max(0, rank_offset) + gemini_checks:]:
                ...
                try:
                    ...
                    if candidate_target.exists() and candidate_target.stat().st_size >= 1024:
                        chosen, target, search_used = candidate, candidate_target, search
                        judge_info = {"fallback_after_rejections": True}
                        break
                except Exception:
                    candidate_target.unlink(missing_ok=True)
...
def ensure_visual_coverage(plan: ShotPlan, manifest: list[dict] | None = None) -> set[int]:
    good = [i for i, scene in enumerate(plan.scenes) if scene.asset and Path(scene.asset).exists() and scene.asset_kind != "blank"]
    if not good:
        return set()
    filled: set[int] = set()
...
def _semantic_rerank(scene: Scene, ranked: list[tuple[AssetCandidate, str]], *, top_k: int) -> None:
    image_pairs = [(candidate, query) for candidate, query in ranked if candidate.kind == "image"][:max(1, top_k)]
    if not image_pairs:
        return
    try:
    ...
    with tempfile.TemporaryDirectory(prefix="video-ai-clip-") as temp_dir:
        ...
        for idx, (candidate, _) in enumerate(image_pairs):
            ...
            try:
                if candidate.local_path:
                    shutil.copy2(candidate.local_path, path)
                else:
        ...
        for candidate, similarity in zip(valid, scores):
            ...
            candidate.score += float(similarity) * 28.0
